...
if(!class_exists("JoomlaImage")){
	class JoomlaImage
	{
		...
		public function createThumbs($thumbSizes, $creationMethod = self::SCALE_INSIDE, $thumbsFolder = null)
		{
			...
			if ($thumbs = $this->generateThumbs($thumbSizes, $creationMethod))
			{
				...
				foreach ($thumbs as $thumb)
				{
					...
					{
						// Return JoomlaImage object with thumb path to ease further manipulation
		...
		public function crop($width, $height, $left = null, $top = null, $createNew = true)
		{
			...
			// If we are cropping to a new image, create a new JoomlaImage object.
			if ($createNew)
			{
				// @codeCoverageIgnoreStart
				$new = new JoomlaImage($handle);
		...
		/**
		 * Method to load a file into the JoomlaImage object as the resource.
		 *
		...
		{               
			...
			// If we are resizing to a new image, create a new JoomlaImage object.
			if ($createNew)
			{          
				// @codeCoverageIgnoreStart
				$new = new JoomlaImage($handle);
		 ...
		 *
		 * @return  object  JoomlaImage Object for chaining.
		 *
		 * @since   12.3
		 */
		...
		protected function getFilterInstance($type)
		{
			...
			$className = 'JoomlaImageFilter' . ucfirst($type);
			...
			if (!($instance instanceof JoomlaImageFilter))
			{
				// @codeCoverageIgnoreStart
